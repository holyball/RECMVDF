'''
Description: MVUGCForest的交叉验证
             0520: 增加单个view的结果
             0606: 实现-跑多次取最好
Author: tanqiong
Date: 2023-05-13 23:20:33
LastEditTime: 2023-07-25 21:27:16
...
def get_config():
    ...
    config["enforce_intra"] = False                             # 是否强迫生成intra-section feature
...
result_path = args.result_path if args.result_path is not None else "../result/MVUGCForest_4views_0725_AUPR_NotEnforce"
...
if __name__=="__main__":
    ...
    for i, (x_train, x_test, y_train, y_test) in enumerate(split_multiview_data_cv(features_dict, labels, cv)):
        ...
        # 重命名文件夹
        new_best_finding_dir_name = os.path.join(os.path.dirname(best_finding_dir), "finding_best")
        # shutil.move(best_finding_dir, new_best_finding_dir_name)
        os.rename(best_finding_dir, new_best_finding_dir_name)
