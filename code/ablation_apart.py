...
LastEditTime: 2023-10-12 20:00:51
LastEditors: tanqiong
'''

# 完整数据集上(dataset_0403)的实验
from MVRECDF.MVUGCForest import MVUGCForest, joint_multi_opinion
...
def set_ablation_configs(path_prefix):
    """设置消融实验的参数"""
    ...
    config_5["logger_path"] = os.path.join(path_prefix, "all_drop")
    param_dict["all_drop"] = config_5

    return param_dict

path_prefix = args.result_path if args.result_path is not None else "/data/tq/RECOMB2024/result/消融实验_apart"
            ...
            for c in combinations(range(len(features_dict)), n_views):
                ...
                for i, (x_train, x_test, y_train, y_test) in enumerate(split_multiview_data_cv(features_train_dict, labels, cv)):
                    ...
                    for finding in range(1):
                        ...
                        # 增加logger路径
                        config["logger_path"] = os.path.join(
                            fold_dir
                        )
                        ...
                        aupr_cur = eval_func(y_test, y_pred_proba_cur)
                        if (aupr_cur<aupr_worst):
                            aupr_worst = aupr_cur
                    ...
                    # 重命名文件夹
                    new_best_finding_dir_name = os.path.join(os.path.dirname(best_finding_dir), "worst")
